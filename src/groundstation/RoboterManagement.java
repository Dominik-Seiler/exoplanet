...
public class RoboterManagement extends Thread {
	...
	public void run() {
		...
		try {
			while((messageFromRobot = in.readLine()) != null) {
...
//					bs.saveMeasure(robot, split[1], split[2]);
					bs.createRestRequest("POST", "http://localhost8080/api/v1/messdaten", new Messdaten(robot.getPlanetId(), robot.getX(), robot.getY(), split[1], Double.parseDouble(split[2])));
					
				}else if(split[0].contains("scaned:MEASURE")) {
					bs.createRestRequest("POST", "http://localhost8080/api/v1/messdaten", new Messdaten(robot.getPlanetId(), getNewX(), getNewY(), split[1], Double.parseDouble(split[2])));
					
				}else if(split[0].contains("moved:POSITION")) {
					robot.setX(Integer.parseInt(split[1]));
					robot.setY(Integer.parseInt(split[2]));
					bs.createRestRequest("PUT", "http://localhost8080/api/v1/roboter/" + robot.getId(), robot);
